...
def load_roads(
    ...
    if path.exists():
        ...
        for row in base.itertuples(
            ...
            for dx in (-1, 0, 1):
                for dy in (-1, 0, 1):
                    rows.append(
                        {
                            "grid_x": row.grid_x + dx,
                            "grid_y": row.grid_y + dy,
                        }
                    )
        ...
        catalog["grid_id"] = (
            catalog["grid_x"].astype(str)
            + "_"
            + catalog["grid_y"].astype(str)
        )
...
def build_daily_panel(
    ...
    weather = engineer_weather_features(
        weather
    )
    ...
    min_date = (
        ...
        if start_date
        else weather["date"].min()
    )

    max_date = (
        pd.Timestamp(end_date)
        if end_date
        else weather["date"].max()
    )
    ...
    panel = add_days_since_last_repair(
        panel,
        repairs,
    )
...
def add_days_since_last_repair(
    panel: pd.DataFrame,
    repairs: pd.DataFrame,
) -> pd.DataFrame:
    out = panel.copy()
...
def risk_level_from_percentile(
    percentile: float,
) -> str:
    if percentile > 0.95:
        return "매우 높음"
...
def _reason_text(
    ...
    if group == "precipitation":
        return (
            ...
            f"7일 "
            f"{row['precip_7d']:.1f}mm)"
        )

    if group == "freeze_thaw":
        return (
            f"최근 7일 동결·융해 "
            f"{row['freeze_thaw_7d']:.0f}회"
        )
    ...
    if group == "recent_recurrence":
        return (
            f"최근 30일 포트홀 "
            f"{row['past_potholes_30d']:.0f}건"
        )

    if group == "long_term_history":
        return (
            f"과거 누적 포트홀 "
            f"{row['past_potholes_total']:.0f}건"
        )

    if group == "repair":
        return (
            f"보수 후 "
            f"{row['days_since_last_repair']:.0f}일 "
            f"경과 영향"
        )
