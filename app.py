...
from src.features import risk_level
...
from src.update_weather_forecast import refresh_weather_forecast
...
def normalize_predictions(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if "risk_percentile" not in out:
        if "prediction_date" in out.columns:
            out["risk_percentile"] = (
                out.groupby("prediction_date")["risk_score"]
                .rank(method="average", pct=True)
            )
        else:
            out["risk_percentile"] = out["risk_score"].rank(
                method="average",
                pct=True,
            )

    if "risk_level" not in out:
        out["risk_level"] = out["risk_percentile"].map(
...
try:
    forecast_updated = refresh_weather_forecast(
...
if need_prediction_update:

    try:
        with st.spinner(
            "최신 기상예보를 반영해 "
            "향후 3일 도로 위험도를 계산하고 있습니다..."
        ):
            run_prediction(
                str(CONFIG_PATH),
...
if not prediction_path.exists():
    st.error(
        "예측 파일이 없습니다. "
        "`python -m src.predict --config config.yaml --horizon 3`"
        "을 먼저 실행하십시오."
    )
    st.stop()


all_pred = normalize_predictions(
...
st.caption(
    "최신 기상예보를 반영한 전북 500m 격자별 "
    "향후 3일 포트홀 상대 위험도 · "
    "모델 위험 + 재발 위험 + 도로 중요도 기반 보수 우선순위"
)


if "prediction_date" in all_pred.columns:

    available_prediction_dates = sorted(
        all_pred["prediction_date"]
        .astype(str)
        .dropna()
        .unique()
    )
    ...
    selected_prediction_date = st.selectbox(
        "위험도 예측 날짜",
        available_prediction_dates,
        index=0,
    )

    pred = all_pred[
        all_pred["prediction_date"].astype(str)
...
if pred.empty:
    st.error(
        f"{selected_prediction_date} 예측 데이터가 없습니다."
    )
    st.stop()


previous_prediction_date = st.session_state.get(
    "selected_prediction_date"
)

if (
    previous_prediction_date is not None
    and previous_prediction_date != selected_prediction_date
):
...
if not kakao_key:
    st.warning(
        ...
        f"`{key_env_name}=발급받은_JavaScript_키`를 입력하십시오. "
        "목록은 키 없이도 동작합니다."
    )
...
if route_submitted:
    ...
    if not kakao_rest_key:
        ...
        st.error(
            f"길찾기에는 REST API 키가 필요합니다. "
            f"`.env`에 "
            f"`{rest_key_env_name}=발급받은_REST_API_키`를 "
            "추가하십시오."
        )
        ...
        try:
            ...
            with st.spinner(
                "대안 경로와 포트홀 위험 구간을 비교하고 있습니다..."
            ):

                st.session_state[
                    "safe_route_plan"
                ] = build_safe_route_plan(
                    origin_query,
                    destination_query,
                    kakao_rest_key,
                    pred,
...
route_plan = st.session_state.get(
    "safe_route_plan"
)
...
if route_plan:
    ...
    route_m2.metric(
        "예상 시간",
        f"{recommended['duration_min']:.0f}분",
    )
    ...
    route_m4.metric(
        "고위험 격자",
        f"{recommended['high_risk_count']}개",
    )


    road_names = (
        recommended.get("road_names")
        or []
    )
...
with tab_components:
    ...
    st.caption(
        "교통량/도로중요도가 없으면 "
        "0.75:0.15 가중치를 합이 1이 되도록 재정규화합니다."
    )
...
with tab_model:
    ...
    if metrics_path.exists():
        ...
        m2.metric(
            "Test PR-AUC",
            f"{test_metrics.get('pr_auc', 0):.3f}",
        )
...
with tab_raw:
    ...
    st.download_button(
        ...
        file_name=(
            f"road_doctor_predictions_"
            f"{selected_prediction_date}.csv"
        ),
        mime="text/csv",
    )